...
class Tile38Uploader():
    ...
    def upload_to_server(self, filename):
        ...
        traffic_json = json.loads(traffic_json)[0]
        for timestamp in self.timestamps: 
            current_timestamp_readings = traffic_json[timestamp];
            for current_reading in current_timestamp_readings:
                icao_address = current_reading['icao_addresss']
                traffic_source = 3
                source_type = 0
                lat_dd = current_reading['lat']
                lon_dd = current_reading['lon']
                time_stamp = current_reading['timestamp']
                altitude_mm = current_reading['altitude']
                
                result = self.client.execute_command('SET', 'fleet', icao_address, 'FIELD', 'traffic_source ', traffic_source , 'FIELD', 'source_type' ,source_type, 'FIELD', 'time_stamp' ,time_stamp,'POINT', lon_dd, lat_dd,altitude_mm)
...
if __name__ == '__main__':
    ...
    my_uploader.upload_to_server(filename='micro_flight_data.json')